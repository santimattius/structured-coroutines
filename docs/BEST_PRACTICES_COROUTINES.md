...
| 4.1 | Loops without cooperation                   | ✅ Warning         | ✅ LoopWithoutYield                            | ✅                       | ✅ + Quick fix          | Compiler: LOOP_WITHOUT_YIELD for for/while in suspend without delay/yield/ensureActive; Gradle option loopWithoutYield. IDE quick fixes: ensureActive(), yield(), delay(0). |
...
| 4.6 | withTimeout scope cancellation              | ❌                 | ✅ WithTimeoutScopeCancellation (⚠️ heuristic) | ⚠️ Heuristic            | ✅ WithTimeoutScopeCancellation (⚠️ heuristic) | Detekt/IDE: withTimeout not in try/catch covering TimeoutCancellationException/CancellationException/Exception/Throwable; CANCEL_006 + doc link; IDE quick fix replaces with withTimeoutOrNull. FPs if caught in upper layer. |
...
| 5.3 | CoroutineExceptionHandler / launch vs async | ❌                 | ❌                                             | ❌                       | ✅ Doc in AsyncWithoutAwait | EXCEPT_003 reference added to async-without-await inspection description; explains that async exceptions are deferred until await() and don't reach CEH. |
| 6.1 | runBlocking + delay in tests                | ❌                 | ✅ RunBlockingWithDelayInTest                  | ✅                       | ✅ Intention           | Detekt/Lint: test files (name/package). IDE: intention “Convert to runTest”.                  |
...
| 8.2 | Lifecycle-aware Flow (Android)              | ❌                 | ❌                                             | ✅ LifecycleAwareFlowCollection | ✅ LifecycleAwareFlowCollection | Lint and IntelliJ: detect flow collect in lifecycleScope without repeatOnLifecycle/flowWithLifecycle; ARCH_002 + doc link.                       |
...
| **Detekt**       | 1.1–1.3, 2.1, 2.2, 3.1, 3.3, 3.4, 4.1, 4.3–4.6, 5.1, 5.2, 6.1, 6.2, 7.1, 7.2 | 1.4, 6.3, 3.5                               | 9.4 optional                                                 |
| **Android Lint** | Same as Detekt (minus 4.6) + 3.1 Main, 8.2 lifecycle Flow, 7.1, 7.2 | 1.4, 4.5, 4.6, 6.2, 6.3                     | 4.6 heuristic detector                                       |
| **IntelliJ**     | Parity with Compiler/Lint + quick fixes/intentions + 4.6 + 5.3 doc | 1.4, 6.3, 7.1, 7.2, 9.1 | —                                                            |
